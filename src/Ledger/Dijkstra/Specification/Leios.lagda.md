...
A registered voting key is honoured until `maxKeyAgeEpochs`{.AgdaField} epochs
after its registration.  Expiry is judged against the epoch the committee is
...
selectCommittee pp e pd pools =
  ...
  where
    open PParams pp using (leiosCommitteeSize; maxKeyAgeEpochs)
